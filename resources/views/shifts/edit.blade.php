<x-layout>
    ...
    @if ((isset($response) && $response == "failed") || !isset($response))
        <form action="/shifts/edit/{{ $shiftid }}/update" method="post">
            @csrf
            <div class="mb-3">
                ...
                @elseif (isset($shift->date) && $shift->date != "")
                    <input type="date" class="form-control" name="date" id="txtdate" value="{{ substr($shift->date, 0, 10) }}" />
                @else
            ...
            <div class="mb-3">
                <div class="me-3 d-inline-block">
                    ...
                    @elseif (isset($shift->employee) && $shift->employee != "")
                		<input type="text" class="form-control" name="employee" id="txtemployee" value="{{ $shift->employee }}" />
                    @else
                ...
                <div class="d-inline-block">
                    ...
                    @elseif (isset($shift->employer) && $shift->employer != "")
                		<input type="text" class="form-control" name="employer" id="txtemployer" value="{{ $shift->employer }}" />
                    @else
            ...
            <div class="mb-3">
                <div class="me-3 d-inline-block">
                    ...
                    @elseif (isset($shift->hours) && $shift->hours != "")
                        <input type="text" class="form-control" name="hours" id="txthours" value="{{ $shift->hours }}" />
                    @else
                ...
                <div class="d-inline-block">
                    ...
                    @elseif (isset($shift->rate_per_hour) && $shift->rate_per_hour != "")
                        <input type="text" class="form-control" name="rate_per_hour" id="txtrateperhour" value="{{ $shift->rate_per_hour }}" />
                    @else
            ...
            <div class="mb-3">
                <div class="me-3 d-inline-block">
                    ...
                    <select class="form-select" name="taxable" id="seltaxable">
                        <option value="Yes" {{ (isset($input->taxable) && $input->taxable == "Yes") || (isset($shift->taxable) && $shift->taxable == "Yes") ? "selected" : "" }}>Yes</option>
                        <option value="No" {{ (isset($input->taxable) && $input->taxable == "No") || (isset($shift->taxable) && $shift->taxable == "No") ? "selected" : "" }}>No</option>
                    </select>
                </div>
                <div class="me-3 d-inline-block">
                    <label class="form-label" for="selstatus">Status</label>
                    <select class="form-select" name="status" id="selstatus">
                        <option value="Complete" {{ (isset($input->status) && $input->status == "Complete") || (isset($shift->status) && $shift->status == "Complete") ? "selected" : "" }}>Complete</option>
                        <option value="Failed" {{ (isset($input->status) && $input->status == "Failed") || (isset($shift->status) && $shift->status == "Failed") ? "selected" : "" }}>Failed</option>
                        <option value="Pending" {{ (isset($input->status) && $input->status == "Pending") || (isset($shift->status) && $shift->status == "Pending") ? "selected" : "" }}>Pending</option>
                        <option value="Processing" {{ (isset($input->status) && $input->status == "Processing") || (isset($shift->status) && $shift->status == "Processing") ? "selected" : "" }}>Processing</option>
                    </select>
                </div>
                <div class="d-inline-block">
                    <label class="form-label" for="selshifttype">Shift Type</label>
                    <select class="form-select" name="shift_type" id="selshifttype">
                        <option value="Day" {{ (isset($input->shift_type) && $input->shift_type == "Day") || (isset($shift->shift_type) && $shift->shift_type == "Day") ? "selected" : "" }}>Day</option>
                        <option value="Holiday" {{ (isset($input->shift_type) && $input->shift_type == "Holiday") || (isset($shift->shift_type) && $shift->shift_type == "Holiday") ? "selected" : "" }}>Holiday</option>
                        <option value="Night" {{ (isset($input->shift_type) && $input->shift_type == "Night") || (isset($shift->shift_type) && $shift->shift_type == "Night") ? "selected" : "" }}>Night</option>
                    </select>
                </div>
            </div>
            <div class="mb-3">
                ...
                @elseif (isset($shift->paid_at) && $shift->paid_at != "")
                    <input type="datetime-local" class="form-control" name="paid_at" id="txtpaidat" value="{{ $shift->paid_at }}" />
                @else
